  ...
  ∀ d' i, decodeCtag d = some (d', i) →
    d'.input = d.input ∧ d'.pos > d.pos ∧ d'.pos ≤ d.input.size := by
      ...
      · -- (d'', 6, 102)
        rename_i d'' hdh
        simp only [Option.bind_eq_bind, Option.bind_eq_some_iff] at h
        obtain ⟨⟨d''', m, n⟩, hdh2, hrest⟩ := h
        split at hrest
        · rename_i hmn
          cases hint : decodeIntL d''' with
          | none => simp [hint] at hrest
          | some res =>
            rcases res with ⟨ra, rb⟩
            simp [hint] at hrest
            obtain ⟨heqs, _⟩ := hrest
            have ⟨hm, hn⟩ : m = 4 ∧ n = 2 := hmn
            subst hm hn
            have ⟨h1, h2, h3⟩ := decodeHead_consumes d   d''  6 102 hdh
            have ⟨h4, h5, h6⟩ := decodeHead_consumes d'' d''' 4   2 hdh2
            have ⟨h7, h8, h9⟩ := decodeIntL_consumes  d''' ra rb hint
            subst heqs
            refine ⟨?_, ?_, ?_⟩
            · rw [h7, h4, h1]
            · omega
            · rw [h4] at h9; rw [h1] at h9; exact h9
        · simp at hrest
      · -- (d'', 6, i_val) with i_val ≠ 102
        rename_i d'' i_val hne hdh
        split at h
        · simp at h
          obtain ⟨h1, _⟩ := h
          subst h1
          exact decodeHead_consumes d d'' 6 i_val hdh
        · split at h
          · simp at h
            obtain ⟨h1, _⟩ := h
            subst h1
...
mutual
  ...
  partial def decodeList : Nat → DecodeState → Option (DecodeState × List Data)
    | .zero  , d => .some (d, [])
    | .succ p, d => match decodeDataLoop d with
        | some (d', x) => match decodeList p d' with
            | some (d'', l) => some (d'', x :: l)
            | none => none
        | none => none
  ...
  partial def decodeListIndef (d : DecodeState) : Option (DecodeState × List Data) :=
    ...
    | _ =>
        match decodeDataLoop d with
        | some (d', x) => match decodeListIndef d' with
            | some (d'', l) => some (d'', x :: l)
            | none => none
        | none => none
  ...
  partial def decodePairList : Nat → DecodeState → Option (DecodeState × List (Data × Data))
    | .zero  , d => .some (d, [])
    | .succ p, d => match decodeDataLoop d with
        | some (d', k) => match decodeDataLoop d' with
            | some (d'', v) => match decodePairList p d'' with
                | some (d''', l) => some (d''', (k, v) :: l)
                | none => none
            | none => none
        | none => none
  ...
  partial def decodeConstr (d : DecodeState) : Option (DecodeState × Data) :=
    ...
    | some (d', i) => match decodeAlternative decodeIndef decodeHead d' with
        | some (d'', r) => match r with
